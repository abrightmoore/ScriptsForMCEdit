...
def CubeIter(level, block, (x1,y1,z1), (x2,y2,z2)):
	MAXSIZE = 2
	CHANCE = 2
	...
	if x2-x1 > MAXSIZE and y2-y1 > MAXSIZE and z2-z1 > MAXSIZE: # If the cube isn't too small
		Cube(level, block, (x1,y1,z1), (x2,y2,z2) ) # Draw this cube
	
		# For each octet of the cube, randomly go deeper, until a limit is reached.
		
		if randint(0,CHANCE) == 1:
			CubeIter(level, block, (x1,y1,z1), (x1+halfWidth, y1+halfHeight, z1+halfDepth) )
		if randint(0,CHANCE) == 1:
			CubeIter(level, block, (x1+halfWidth,y1,z1), (x2, y1+halfHeight, z1+halfDepth) )
		if randint(0,CHANCE) == 1:
			CubeIter(level, block, (x1,y1+halfHeight,z1), (x1+halfWidth, y2, z1+halfDepth) )
		if randint(0,CHANCE) == 1:
			CubeIter(level, block, (x1,y1,z1+halfDepth), (x1+halfWidth, y1+halfHeight, z2) )
		if randint(0,CHANCE) == 1:
			CubeIter(level, block, (x1+halfWidth,y1+halfHeight,z1), (x2, y2, z1+halfDepth) )
		if randint(0,CHANCE) == 1:
			CubeIter(level, block, (x1,y1+halfHeight,z1+halfDepth), (x1+halfWidth, y2, z2) )
		if randint(0,CHANCE) == 1:
			CubeIter(level, block, (x1+halfWidth,y1,z1+halfDepth), (x2, y1+halfHeight, z2) )
		if randint(0,CHANCE) == 1:
			CubeIter(level, block, (x1+halfWidth,y1+halfHeight,z1+halfDepth), (x2, y2, z2) )
...
def Cubes(level, box, options):
	...
	CubeIter(level, (blockID,blockData), (box.minx, box.miny, box.minz), (box.maxx, box.maxy, box.maxz))
