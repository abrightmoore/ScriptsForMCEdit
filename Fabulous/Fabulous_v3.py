...
inputs = (
		("FABULOUS", "label"),
		("Method", ("Horizontal","Vertical","Spherical","Diagonal","Sinusoidal","Check")),
		...
		for iterX in xrange(0, width):
			...
			for iterY in xrange(0, height):
				for iterZ in xrange(0, depth):
					tempBlock = level.blockAt(box.minx + iterX, box.miny + iterY, box.minz + iterZ)
					if tempBlock != 0 and not (tempBlock in ignoreList):
						dx = (centreWidth - iterX) % len(colours)
						dy = (centreHeight - iterY) % len(colours)
						dz = (centreDepth - iterZ) % len(colours)
						r = dx
						if dz > dy and dz > dx:
							r = dz
