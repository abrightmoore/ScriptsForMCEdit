...
inputs = (
	  ...
	  ("Support Material:", alphaMaterials.WoodPlanks), # https://github.com/mcedit/pymclevel/blob/master/materials.py
	  ("Support Gap:", 0),
	  ("Start X:", 0),
	  ("Start Y:", 64),
	  ("Start Z:", 0),
...
def councilWorks(level, box, options):
	...
	for iter in xrange(-hWidth,hWidth):
		...
		for iterY in xrange(0,bHeight):
			...
			if iterY > 0:
				theMaterial = AIR # Tunnels
			drawLine(level, theMaterial, (x1, y1+iterY, z1), getRelativePolar( (x1,y1+iterY,z1), (theta, phi, distance) ) )
	
	if supportGap < 1:
		supportGap = randint(2,16)
	
	if TUNNELSUPPORT == True or ARCHES == True:
		print '%s: Tunnel Supports or Arches' % (method)
		for iterP in xrange(0,(int)(distance)):
			if iterP%supportGap == 0: # To Do: Randomise the gap each run?
				(x1, y1, z1) = getRelativePolar( (startX,startY,startZ), (theta, phi, iterP) )
				theBlock = level.blockAt((int)(x1),(int)(y1+bHeight),(int)(z1))
#				print '%s' % (theBlock)
				if ARCHES == True or theBlock != 0: # To Do: ignore list?
					for iterY in xrange(1,bHeight):
						for iter in xrange(-hWidth,hWidth):
							# if iterY >= bHeight-1 or abs(iter) >= hWidth-1:   # (ARCHES == False and iterY >= bHeight-1) or 
							if (iterY >= (int)((bHeight-hWidth)/2) and bHeight-iterY <= (abs)(iter*2)) or abs(iter) >= hWidth-1:
								(x2, y2, z2) = getRelativePolar( (x1,y1+iterY,z1), (theta+pi/2, 0, iter) )
								setBlock(level, supportMaterial, (int)(x2), (int)(y2), (int)(z2))
