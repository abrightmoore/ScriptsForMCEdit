...
def councilWorks(level, box, options):
	...
	if TUNNELSUPPORT == True:
		print '%s: Tunnel Supports' % (method)
...
#				print '%s' % (theBlock)
				if theBlock != 0: # To Do: ignore list?
					print '%s' % (theBlock)
					for iterY in xrange(1,bHeight):
						for iter in xrange(-hWidth,hWidth):
							# if iterY >= bHeight-1 or abs(iter) >= hWidth-1:
							if iterY >= bHeight-1 or (iterY >= bHeight-hWidth and bHeight-iterY == (abs)(iter)) or abs(iter) >= hWidth-1:
