		...
		if y == (height-1-yardist) or y == (height-1-3*yardist) or y == (height-1-6*yardist):
			for z in xrange(int(LENGTHQUANTUM/2),depth):
				if getBlock(level, box.minx+centreWidth, box.miny+y, box.minz+z+1) != AIR and getBlock(level, box.minx+centreWidth, box.miny+y, box.minz+z) == AIR:
					...
					if options["Rigging?"] == True and y == (height-1-6*yardist): # Lowest yard only
						...
						if (startx,starty,startz) != (0,0,0) and (endx,endy,endz) != (0,0,0):
							...
							for iz in xrange(startz, endz):
								while ix < centreWidth and found == False: 
									block = getBlock(level, box.minx+startx+HULLINSET+ix,box.miny+starty-1,box.minz+startz)
									if block == AIR:
										found = False
										setBlock(level,OAKPLANKS,box.minx+startx+HULLINSET+ix,box.miny+starty-1,box.minz+startz)
									else:
										found = True
									ix = ix+1
							for ix in xrange(-2,1): # platform around the mast
								for iz in xrange(-2,1):
									setBlock(level,OAKPLANKS,box.minx+centreWidth+ix, box.miny+y+2, box.minz+z+2+iz)
					drawLineForced(level, WOODEW,(box.minx+centreWidth, box.miny+y, box.minz+z), (box.minx+((centreWidth/2)/((height-y)/yardist)), box.miny+y, box.minz+z))
