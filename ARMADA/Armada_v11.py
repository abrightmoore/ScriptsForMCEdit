...
def drawSquarePrism(level,material,sz,ht,x,y,z):
	for iy in xrange(y,y+ht):
		for ix in xrange(x,x+sz):
			for iz in xrange(z,z+sz):
#				doIt = True
#				if (ix == x) or (ix == x+sz-1) or (iz == z) or (iz == z+sz-1) and randint(1,100) > 80:
#					doIt = False
#				if doIt == True:
				setBlock(level,material,ix,iy,iz)
#				if doIt == False:
#					setBlock(level,(89,0),ix,iy,iz) # Glowstone highlights
				
def contowers(level,box,RAND,material):
	print "ConTowers"
	z = box.minz
	width = box.maxx-box.minx
	depth = box.maxz-box.minz
	height = box.maxy-box.miny
	while z < box.maxz-int(width/8):
		maxHeightHere = float(depth-z)/float(depth)*float(height)
		# max height tapers to the edges too
		cx = box.minx+int(width/2) # CentreX
		numStructs = RAND.randint(1,int(width/2))
		for i in xrange(0,numStructs):
			sz = RAND.randint(4,16) # Player scale?
			if width-sz > 0:
				x = box.minx + RAND.randint(0,width-sz) # start pos for the structure
				dx = abs(x+sz/2-box.minx-width/2) # Distance of centre of structure from the centre line
				maxHeightRightHere = cos(float(dx)/(width/2)*pi/2)*maxHeightHere
				# Draw something!
				if int(maxHeightRightHere) > 2:
					drawSquarePrism(level,material,sz,RAND.randint(1,int(maxHeightRightHere)),x,box.miny,z)
		z = z+RAND.randint(int(width/16),int(width/8)) # Step along to the next row.
...
def spaceShip1(level0,box0,options):
	...
	SMOOTHAMOUNT = RAND.randint(0,SMOOTHAMOUNT)
	
	# BEAM of the ship
	MIDPOINT = [ (centreWidth, centreHeight, centreDepth) ]
	QUANTUM = int(depth/RAND.randint(6,16)) # was /10
...
#			BEAMP.append( curP )
	...
	# Conning towers
	if SEGMENTS > 4 and depth > 200 and RAND.randint(1,100) > 20: # TEST
		(x,y,z) = BEAMP[1]
		b = BoundingBox((x-width/8,y,z),(width/4,centreHeight,(SEGMENTS-RAND.randint(2,SEGMENTS-2))*SEGMENTLENGTH))
		contowers(level,b,RAND,WOOD)
	...
	if RAND.randint(1,100) > 20:
		...
		while y < box.maxy:
			for x in xrange(box.minx,box.maxx):
				...
				for z in xrange(box.minz,box.maxz):
					b = getBlock(level,x,y,z)
					if b != (0,0) and b != (95,15): # Air and glass
						if down == True and RAND.randint(1,100) > 60:
							down = False
						elif down == False and RAND.randint(1,100) > 60:
							down = True
						if down == True and RAND.randint(1,100) > 80:
							if (getBlock(level,x+1,y,z) == (0,0) or getBlock(level,x-1,y,z) == (0,0)) and (getBlock(level,x,y+1,z) != (0,0) and getBlock(level,x,y-1,z) != (0,0)): # next to air
								setBlockForced(level,(89,0),x,y,z) # Glowstone
						
			y = y+RAND.randint(3,11)
...
def perform(level,box, options):
	...
	elif options["Space?"] == "Ship 1":
		spaceShip1(level,shipBox,options)
	elif options["Space?"] == "Ships":
		w = int(depth/2)
		h = int(w/3*2)
		i = 0
		while w*i < width-w-1:
			y = shipBox.miny
			while y < shipBox.maxy-h:
				b = BoundingBox((shipBox.minx+w*i,y,shipBox.minz),(w,h,depth))
				spaceShip1(level,b,options)
				y = y+1.5*h
			i = i+2
