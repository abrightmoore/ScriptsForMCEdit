...
def ALIFEMCPI(level, box, options): 
	...
	MATERIAL = options["Material"] # This is the cell block when alive. When dead, it is air.
	...
	Q = zeros((WIDTH,DEPTH,2)) # -1 = dead, otherwise the number indicates the number of neighbours
	...
	for iterX in xrange(0, WIDTH):
		for iterZ in xrange(0, DEPTH):
			if level.blockAt(box.minx+iterX,box.miny,box.minz+iterZ) != 0: # Not air
				Q[iterX,iterZ,fieldIndex] = 0
			else:
				Q[iterX,iterZ,fieldIndex] = DEAD
	...
	theBlock = block.AIR
	theBlock.id = options["Material"].ID
	theBlock.data = options["Material"].blockData
	...
	for cycling in xrange(0, ITERATIONS):
				...
				for ix in xrange(-1,2):
					for iz in xrange(-1,2):
						if ix == 0 and iz == 0:
							...
							if Q[(iterX+ix)%WIDTH,(iterZ+iz)%DEPTH,fieldIndex%2] != DEAD:
		...
		for iterX in xrange(0,WIDTH):
			for iterZ in xrange(0,DEPTH):
				if Q[iterX,iterZ,fieldIndex] != DEAD: # Now alive. Set to material
					mc.setBlock((int)(playerPos.x+iterX),(int)(playerPos.y+HEIGHTOFFSET),(int)(playerPos.z+iterZ),theBlock) # ToDo: Use setBlocks for a boost
				#elif Q[iterX,iterZ,fieldIndex] == DEAD and Q[iterX,iterZ,(fieldIndex+1)%2] != DEAD: # Was alive, now dead. Set to air
				else: # is Dead
					mc.setBlock((int)(playerPos.x+iterX),(int)(playerPos.y+HEIGHTOFFSET),(int)(playerPos.z+iterZ),block.GLASS) # ToDo: Use setBlocks for a boost
	...
	for iterX in xrange(0,WIDTH):
		for iterZ in xrange(0,DEPTH):
			if Q[iterX,iterZ,fieldIndex] != DEAD: # Now alive. Set to material
				...
				setBlock(level,(theBlock.id,theBlock.data),(int)(box.minx+iterX),(int)(box.miny),(int)(box.minz+iterZ)) # ToDo: Use setBlocks for a boost
				...
				setBlock(level,(0,0),(int)(box.minx+iterX),(int)(box.miny),(int)(box.minz+iterZ)) # ToDo: Use setBlocks for a boost
