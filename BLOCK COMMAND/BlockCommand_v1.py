	...
	for iterY in xrange(0, height):
		for iterX in xrange(0, width):
			for iterZ in xrange(0, depth):
				...
				(ID, Data) = tempBlock
				if tempBlock != AIR:
					spawnerX = baseX + (packedSpawnerCount%PACKEDCHUNKSIZE)*2
					spawnerY = baseY + ((int)(packedSpawnerCount/(PACKEDCHUNKSIZE*PACKEDCHUNKSIZE)))*2
					spawnerZ = baseZ + ((int)(packedSpawnerCount/PACKEDCHUNKSIZE)%PACKEDCHUNKSIZE)*2
					...
					chunk.TileEntities.append( 	createCommandBlockData(spawnerX, 
									spawnerY, 
									spawnerZ, 
									"/setblock "+str(box.minx+iterX)+" "+str(box.miny+iterY)+" "+str(box.minz+iterZ)+" "+str(ID)+" "+str(Data)
									))
