...
def BlockCommand(level, box, options):
	...
	for iterY in xrange(0, height):
		print '%s: Processing layer %s of %s' % (method, iterY, height)
		for iterX in xrange(0, width):
			for iterZ in xrange(0, depth):
				...
				if tempBlock != AIR or ALLAIR == True:
					...
					if tempBlock != SPONGE: # https://github.com/mcedit/pymclevel/blob/master/materials.py
					...
					if COORDS == True:
						...
						cY = cY - spawnerY
						cZ = cZ - spawnerZ 
						theCommand = "/"+CBCOMMAND+" ~"+str(cX)+" ~"+str(cY)+" ~"+str(cZ)+" "+str(ID)+" "+str(Data)+" "+CBOPERATION
					else:
						theCommand = "/"+CBCOMMAND+" "+str(cX)+" "+str(cY)+" "+str(cZ)+" "+str(ID)+" "+str(Data)+" "+CBOPERATION
