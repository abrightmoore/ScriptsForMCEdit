# This filter replicates the pattern in the selection area in areas that correspond to the non-air blocks in the selection area. tl;dr: recursive FRACTALS!
...
inputs = (
	  ("FRACTAL", "label"),
	  ("Select an area. Execute it.", "label"),
	  ("(This filter is not selection-safe)", "label"),
...
# Utility methods
def fill(level,block,(oX, oY, oZ ),(width,height,depth)):
	for iterY in xrange(oY,oY+height):
		for iterZ in xrange(oZ,oZ+depth):
			for iterX in xrange(oX,oX+width):
				setBlock(level,block,iterX,iterY,iterZ)
		...
		for x in xrange(0,width):
			for z in xrange(0,depth):
				...
				if tempBlock != AIR:
					...
					if Solid == False or (Solid == True and tempBlockAbove == AIR):
						level.copyBlocksFrom(schematic, BoundingBox((0,0,0),(width,height,depth)), (box.minx+XModifier, box.miny+YModifier, box.minz+ZModifier ))
					elif Solid == True and tempBlockAbove != AIR:
						fill(level,tempBlockAbove,(box.minx+XModifier, box.miny+YModifier, box.minz+ZModifier ),(width,height,depth))
