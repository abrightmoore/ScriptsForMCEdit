...
inputs = (
	  ...
	  ("Colours:", ("string","value=14 1 6 2 4 13 5 9 3 11 10 2 1 8 7 15 12")),
	  ("Number of Steps", 1000),
	  ("Vertical Magnification", 5.0),
	  ("Horizontal Magnification", 5.0),
	  ("Start X", 1.0),
	  ("Start Y", 1.0),
	  ("Start Z", 1.0),
	  ("R", 24.74),
...
def Lorentz(level, box, options):
	...
	while STEPNUM > 0:
		...
		drawLine(level,(BLOCKID, coloursList[colour]),
				 (box.minx+centreWidth+y*HORIZMAGSCALE, box.miny+centreHeight+x*VERTMAGSCALE, box.minz+centreDepth+z*HORIZMAGSCALE),
				 (box.minx+centreWidth+(y+dy)*HORIZMAGSCALE, box.miny+centreHeight+(x+dx)*VERTMAGSCALE, box.minz+centreDepth+(z+dz)*HORIZMAGSCALE)
				)
