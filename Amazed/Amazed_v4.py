...
inputs = (
 ...
 # 	  ("Material:", alphaMaterials.Cobblestone),
	  ("Cell Size:", 4),
	  ("Wall Purge %:", 0.0),
	  ("Material (Floor):", alphaMaterials.Cobblestone),
	  ("Material (Roof):", alphaMaterials.Cobblestone),
	  ("Material (North):", alphaMaterials.Cobblestone),
	  ("Material (South):", alphaMaterials.Cobblestone),
	  ("Material (East):", alphaMaterials.Cobblestone),
	  ("Material (West):", alphaMaterials.Cobblestone),
...
def Amazed(level, box, options):
	...
	WALLPURGEPERCENT = options["Wall Purge %:"]
#	material = (options["Material:"].ID, options["Material:"].blockData)
	materialR = (options["Material (Roof):"].ID, options["Material (Roof):"].blockData)
	materialF = (options["Material (Floor):"].ID, options["Material (Floor):"].blockData)
	materialN = (options["Material (North):"].ID, options["Material (North):"].blockData)
	materialS = (options["Material (South):"].ID, options["Material (South):"].blockData)
	materialE = (options["Material (East):"].ID, options["Material (East):"].blockData)
	materialW = (options["Material (West):"].ID, options["Material (West):"].blockData)
	...
	# Hack - purge some number of walls from the maze to improve circulation
	mazeSize = width * depth * height
	numberOfWallsToBreak = (int)(WALLPURGEPERCENT * mazeSize) # 5%
	while numberOfWallsToBreak > 0:
		wallDir = randint(1, 6)
		(x,y,z) = (randint(2,width-2),randint(2,height-2),randint(2,depth-2))
		if wallDir == 1:
			cells[x,y,z,1] = NOWALL
			cells[x1,y1,z1,2] = NOWALL
		elif wallDir == 2:
		...
		numberOfWallsToBreak = numberOfWallsToBreak -1
				...
				if cells[iterX, iterY, iterZ, 1] == WALL:
					for iY in xrange(0,WALLSIZE):
						for iZ in xrange(0,WALLSIZE):
							setBlock(level, materialW, box.minx+(iterX*WALLSIZE),box.miny+(iterY*WALLSIZE)+iY,box.minz+(iterZ*WALLSIZE)+iZ)
				if cells[iterX, iterY, iterZ, 2] == WALL:
					for iY in xrange(0,WALLSIZE):
						for iZ in xrange(0,WALLSIZE):
							setBlock(level, materialE, box.minx+(iterX*WALLSIZE)+WALLSIZE-1,box.miny+(iterY*WALLSIZE)+iY,box.minz+(iterZ*WALLSIZE)+iZ)
				if cells[iterX, iterY, iterZ, 3] == WALL:
					for iX in xrange(0,WALLSIZE):
						for iZ in xrange(0,WALLSIZE):
							setBlock(level, materialF, box.minx+(iterX*WALLSIZE)+iX,box.miny+(iterY*WALLSIZE),box.minz+(iterZ*WALLSIZE)+iZ)
				if cells[iterX, iterY, iterZ, 4] == WALL:
					for iX in xrange(0,WALLSIZE):
						for iZ in xrange(0,WALLSIZE):
							setBlock(level, materialR, box.minx+(iterX*WALLSIZE)+iX,box.miny+(iterY*WALLSIZE)+WALLSIZE-1,box.minz+(iterZ*WALLSIZE)+iZ)
				if cells[iterX, iterY, iterZ, 5] == WALL:
					for iX in xrange(0,WALLSIZE):
						for iY in xrange(0,WALLSIZE):
							setBlock(level, materialS, box.minx+(iterX*WALLSIZE)+iX,box.miny+(iterY*WALLSIZE)+iY,box.minz+(iterZ*WALLSIZE))
				if cells[iterX, iterY, iterZ, 6] == WALL:
					for iX in xrange(0,WALLSIZE):
						for iY in xrange(0,WALLSIZE):
							setBlock(level, materialN, box.minx+(iterX*WALLSIZE)+iX,box.miny+(iterY*WALLSIZE)+iY,box.minz+(iterZ*WALLSIZE)+WALLSIZE-1)
