...
#	  ("Leaves:", alphaMaterials.Leaves), # https://github.com/mcedit/pymclevel/blob/master/materials.py
	  ...
	  ("Floor Each:", 1),
	  ("Randomness:", 1),
	  ("Randomness (Vertical):", 4),
	  ("Termites?", False),
	  ("Pipe?", False),
	  ("Fang?", False),
	  ("Rotation?", True), # Disable the rotation for Pyramids
	  ("Orientation (Start - degrees):", 45),
	  ("Faces:", 11),
#	  ("Branches?", False),
...
#	LEAVES = (options["Leaves:"].ID, options["Leaves:"].blockData)
	...
	VERTRANDOMNESS = options["Randomness (Vertical):"]
	...
	ORIENTATION = options["Orientation (Start - degrees):"]
	EDGES = options["Faces:"]
#	BRANCHES = options["Branches?"]
	...
	for y in xrange(0, height):
		...
		if PIPE == False and FANG == False:
			if(randint(0,VERTRANDOMNESS) == 0):
				SideLength = SideLength - 1
				if RANDOMNESS > 0:
					offsetX = offsetX+randint(0,2*RANDOMNESS+1)-RANDOMNESS
					offsetZ = offsetZ+randint(0,2*RANDOMNESS+1)-RANDOMNESS
			if ROTATION == True and (randint(0,4) == 0):
				Orientation = Orientation + randint(1,5)
		elif PIPE == True and FANG == False:
			if RANDOMNESS > 0:
				offsetX = offsetX+randint(0,2*RANDOMNESS)-RANDOMNESS
				offsetZ = offsetZ+randint(0,2*RANDOMNESS)-RANDOMNESS
		elif FANG == True:
				if randint(0,10) < 3:
					SideLength = SideLength - 1
				offsetX = centreWidth/2*sin(2*pi/height*(y+1))+centreWidth/2*sin(pi/2+((pi/2.5)/height*(y+1)))
				#offsetZ = offsetZ+randint(0,2*RANDOMNESS)-RANDOMNESS
