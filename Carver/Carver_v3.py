...
inputs = (
	  ("CARVER", "label"),
	  ("Shape", ("Sphere","Sphere-","Cylinder (Vertical)","Cylinder- (Vertical)","Boulder","Diagonal Vertical++","Diagonal Vertical+-")), #,"Diagonal Vertical--","Diagonal Vertical-+")),
...
def Carver(level, box, options):
	...
	if POSTSHAPE == "Cylinder- (Vertical)": # Trim away the exterior
		...
		slope = (float)(depth/width)
		for iterX in xrange(0,width):
			for iterZ in xrange(0,depth):
				if iterX == 0 or iterZ/iterX > slope:
					for iterY in xrange(0,height):
						setBlock(level, AIR, box.minx+iterX,box.miny+iterY,box.minz+iterZ)
...
#		for iterX in xrange(0,width):
#			for iterZ in xrange(0,depth):
#				if iterX == 0 or (depth-iterZ)/iterX > slope:
#					for iterY in xrange(0,height):
#						setBlock(level, AIR, box.minx+iterX,box.miny+iterY,box.minz+iterZ)
