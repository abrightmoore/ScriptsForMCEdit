...
def gear(level,box,options,RAND):
	...
	for y in xrange(box.miny,box.maxy):
		...
		for x in xrange(box.minx,box.maxx):
			...
			for z in xrange(box.minz,box.maxz):
				...
				theta = atan2(pz,px)				
				d = sqrt(px**2+pz**2)
				if (d >= edgeBevel1 and d <= r) or (d <= edgeBevel2) or (d < edgeBevel1 and layer > 0 and layer < maxLayer):
					plotPoint(level, mat, x, y, z, options["Overwrite:"])
				elif (d > r and d <= r+teethSize and int(floor(theta/toothAngle)) % 2 == 0) or (secondBevel == True and (int(d) == edgeBevel1-2) or (int(d) == edgeBevel2+2)):
					plotPoint(level, mat, x, y, z, options["Overwrite:"])
	...
	# triangular cut outs
	if cutNum > 1 and triangleCuts == True:
