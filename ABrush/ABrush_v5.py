...
inputs = [
		(
			...
			("Brush Name:", ( "lineBRUSH", "lineDashedBRUSH","trackBRUSH","tileSelectionBRUSH","pathSelectionBRUSH","treeRootTrunkBranchBRUSH","treeRootTrunkBRUSH" )),
...
def leafPatternSpars(RAND):
	# a leaf/petal shape is scalloped based on the seed, and mirrored around the axis.
...
def flower(level,box,options,RAND):
	...
	fold = False
	if height >= centreDepth and RAND.randint(1,100) < 96:
		fold = True
	...
	for i in xrange(0,NUMLEAVES):
		...
		for (x,y,z) in LEAF:
			...
			P.append((box.minx+centreWidth+px,box.miny+py,box.minz+centreDepth+pz))
			P.append(P[0])
			(px,py,pz) = (0,y,z)
			if fold == True:
				py = py+temp
...
def treeRootTrunkBRUSH(level,box,options,materials,overwrite,RAND,radius,path,arclength):
	method = "treeRootTrunk"
	(method, (width, height, depth), (centreWidth, centreHeight, centreDepth)) = FuncStart(level,box,options,method) # Log start
	...
	for (x,y,z) in path:
		...
		# Trunk section
		counter = counter+1
		angle = (counter*arclength)/2
		r = abs(cos(angle)*radius)
		mat = materials[1]
...
def beanStalk(level,box,options,RAND):
	method = "Bean Stalk"
	(method, (width, height, depth), (centreWidth, centreHeight, centreDepth)) = FuncStart(level,box,options,method) # Log start
	...
	# First we need a 'control' path around which we curl and twist
	C = []
	C.append((cw,0,cz)) # Centre
	C.append((cw,0,cz)) # Centre
	h = 0
	while h < height:
		x = cw+(RAND.randint(-cw,cw)/4)
		y = cz+(RAND.randint(-cz,cz)/4)
		C.append((x,h,y))
		delta = RAND.randint(-4,10)
		if delta == 0:
			delta = 1
		h = h + delta

	C.append((cw,height-1,cz)) # Centre
	C.append((cw,height-1,cz)) # Centre
	
	# Draw the stalk
	numLines = RAND.randint(1,7)*2+1
	lineAngle = 2.0*pi/numLines
	for i in xrange(0,numLines): # A number of tendrils
		P = []
		p = randomise((cw+cw*cos(lineAngle*i),0,cz+cz*sin(lineAngle*i)),4,RAND)
		P.append(p) # Root start
		P.append(p)
		p = (cw+cw*cos(lineAngle*i)/2,RAND.randint(0,h/10),cz+cz*sin(lineAngle*i)/2)
		P.append(randomise(p,8,RAND))
		
		startPos = RAND.randint(1,2+int(len(C)/5))
		for j in xrange(startPos,len(C)-1):
			P.append(randomise(C[j], 8, RAND)) # Trunk with a wiggle

		brush(level,box,options,RAND,P,options["Brush Name:"])		

	FuncEnd(level,box,options,method)

def randomise(p,delta,R):
	(x,y,z) = p
	x += R.randint(-delta,delta)
	y += R.randint(-delta,delta)
	z += R.randint(-delta,delta)
	return (x,y,z)
...
#	box0 = box
	...
	elif options["Operation:"] == "Bean Stalk":
		beanStalk(level0,box0,options,RAND)
