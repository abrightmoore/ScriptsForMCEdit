...
def BOS(level, box, options):
	...
	SNOWLIMIT = 7 # Bedrock compatible
	# Third pass - plot
	for iterX in xrange(0, width):
		for iterZ in xrange(0, depth):
			snowHeightHere = abs(((float)(theSnowField[iterX][iterZ]) - (float)(theField[iterX][iterZ]))*100/16)
			if snowHeightHere > SNOWLIMIT:
				snowHeightHere = SNOWLIMIT
			elif snowHeightHere < 1:
				snowHeightHere = 1 
			print 'v %s' % ((int)(snowHeightHere))
			setBlockIfEmpty(level, (78, (int)(snowHeightHere)), box.minx+iterX, box.miny+theField[iterX][iterZ]+1, box.minz+iterZ)
