...
def tunnelTwisting(level,box,options,R,(px,py,pz),MAXLEN,theta,phi):
	...
	while keepGoing == True:
		if R.randint(1,100) > 1:
			...
			if R.randint(0,100) > 10: # Increase this likelihood for a big cave
				for k in xrange(0,6):
					tunnelTwisting(level,box,options,R,(px,py,pz),size*2,R.randint(0,MAXANGLES)*a,0)
...
#			phi = phi-R.randint(-1,2)*a*2
		...
		if steps % 100 == 0: # branch!
			if R.randint(0,100) == 1:
				tunnelTwisting(level,box,options,R,(px,py,pz),int(MAXLEN/2),-1,-1)
...
def underworld(level, box, options):
	...
	if OPER == "Tunnels":
		tunnelTwisting(level, box, options, R,(px,py,pz),0,-1,-1)
...
def tunnelBrownian(level,box,options,R,(px,py,pz)):
	...
	for i in xrange(0,length):
		...
		if R.randint(1,1000) == 1:
			tunnelRandom(level,box,options,R,(px,py,pz))
