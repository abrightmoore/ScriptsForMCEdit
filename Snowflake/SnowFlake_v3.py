...
def perform(level, box, options):
	...
	Snowflakes(level, box, options)		
	level.markDirtyBox(box)
	
def Snowflakes(level, box, options):
	...
	SnowflakeSize = 30
	if options["Storm:"] == True and width > SnowflakeSize*2:
		instancesX = width/SnowflakeSize
		instancesZ = depth/SnowflakeSize
		instancesY = (height/SnowflakeSize)
		if instancesY < 1:
			instancesY = 1
		for iterX in xrange(0,instancesX):
			for iterY in xrange(0,instancesY):
				for iterZ in xrange(0, instancesZ):
					Snowflake(level, BoundingBox( (box.minx+iterX*SnowflakeSize+ randint(-SnowflakeSize/6,SnowflakeSize/6),box.miny+centreHeight+iterY*SnowflakeSize+randint(-SnowflakeSize/6,SnowflakeSize/6),box.minz+iterZ*SnowflakeSize+ randint(-SnowflakeSize/6,SnowflakeSize/6)),
								(SnowflakeSize+randint(-SnowflakeSize/6,SnowflakeSize/6), SnowflakeSize/4, SnowflakeSize + randint(-SnowflakeSize/6,SnowflakeSize/6) )), options)
	else:
		...
		if True: # ditch this in final edit ;]
			...
			for iterZ in xrange(-depthHere, depthHere):
				...
				if iterZ == 0 or randint(0,10) < CHANCETOFORM:
					...
					x = (int)(r*cos(phi))
					z = (int)(r*sin(phi))
					if abs(x) < centreWidth and abs(z) < centreDepth:
