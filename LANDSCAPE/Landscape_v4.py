...
def landscape(level, box, options): 
	...
	if options["Orthogonal?"] == False: # Elevation map
		...
		(width, height, pixels1, metadata) = reader.asRGBA8() # @Sethbling
			...
			for y in xrange(0, height):
				...
				if opaque(colour, TRANSPARENCY_T): # @Sethbling
					...
					for iter in xrange(0,(int)(intensity)):
						if INVERT == False:
							setBlock(level, (MATERIALID,theBlockData), box.minx+x, box.miny+iter+BASEHEIGHT, box.minz+y)
						else:
							setBlock(level, (MATERIALID,theBlockData), box.minx+x, box.maxy+BASEHEIGHT-iter-1, box.minz+y)
		...
		minMaxFront = zeros((widthFront,heightFront,2))
		for x in xrange(0,widthFront):
			for y in xrange(0,heightFront):
				minMaxFront[x,y,0] = -1
				minMaxFront[x,y,1] = heightTop

		minMaxSide = zeros((widthSide,heightSide,2))
		for x in xrange(0,widthSide):
			for y in xrange(0,heightSide):
				minMaxSide[x,y,0] = -1
				minMaxSide[x,y,1] = widthTop
		...
		for xTop in xrange(0, widthTop): # Traverse the image
			...
			for zTop in xrange(0, heightTop):
				...
				(topBlock, topBlockData) = closestMaterial(colourTop) # @Sethbling
				if opaque(colourTop, TRANSPARENCY_T): # There is some block at x,z posn. Find and plot all the y's
					for yFront in xrange(0, heightFront): # hehe. Y-fronts!
						...
						if opaque(colourFront, TRANSPARENCY_T) and opaque(colourSide, TRANSPARENCY_T): # Plot this Y
							#print 'Plotting block %s %s %s %s %s' % (xTop, yFront, zTop, transparent(colourFront), colourFront)
							setBlock(level, (MATERIALID,topBlockData), box.minx+xTop, box.miny+yFront, box.minz+zTop)
						    # Mark if this is an edge
							if zTop > minMaxFront[xTop,yFront,0]:
								minMaxFront[xTop,yFront,0] = zTop
							if zTop < minMaxFront[xTop,yFront,1]:
								minMaxFront[xTop,yFront,1] = zTop
							if xTop > minMaxSide[zTop,yFront,0]:
								minMaxSide[zTop,yFront,0] = xTop
							if xTop < minMaxSide[zTop,yFront,1]:
								minMaxSide[zTop,yFront,1] = xTop
		
		if options["Colourise?"] == True: # Add a surface layer of colour to each face
			for iterY in xrange(0, heightFront):
				for iterX in xrange(0, widthSide):
					if minMaxSide[iterX,iterY,0] > -1 and minMaxSide[iterX,iterY,1] < widthTop:
						colour = getPixel(pixelsSide, iterX, heightSide-iterY-1)
						(x, colourBlockData) = closestMaterial(colour)
						setBlock(level, (MATERIALID,colourBlockData), box.minx+minMaxSide[iterX,iterY,0], box.miny+iterY, box.minz+iterX)
						setBlock(level, (MATERIALID,colourBlockData), box.minx+minMaxSide[iterX,iterY,1], box.miny+iterY, box.minz+iterX)
				for iterX in xrange(0, widthFront):
					if minMaxFront[iterX,iterY,0] > -1 and minMaxFront[iterX,iterY,1] < heightTop:
						colour = getPixel(pixelsFront, iterX, heightFront-iterY-1)
						(x, colourBlockData) = closestMaterial(colour)
						setBlock(level, (MATERIALID,colourBlockData), box.minx+iterX, box.miny+iterY, box.minz+minMaxFront[iterX,iterY,0])
						setBlock(level, (MATERIALID,colourBlockData), box.minx+iterX, box.miny+iterY, box.minz+minMaxFront[iterX,iterY,1])
