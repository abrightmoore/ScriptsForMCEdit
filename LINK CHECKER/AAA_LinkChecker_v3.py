...
def LinkChecker(level, box, options):
	...
	#	 If it isn't air then sound the alarm - we're not in Kansas any more. #DONE
		...
		for t in chunk.TileEntities:
			...
			if x >= box.minx and x < box.maxx and y >= box.miny and y < box.maxy and z >= box.minz and z < box.maxz:
				if t["id"].value == "Control":
					...
					if command.find(CBCOMMAND) != -1: # @Texelelf
						print 'Found the command %s' % (CBCOMMAND)
						#ignore minecraft:air. These are probably not of interest as they simply toggle/clear
						if command.find("minecraft:air") == -1: # @Texelelf
							...
							if command.find(ALIASPREFIX) == -1: # @Texelelf
								# Mojang have the format setblock <x> <y> <z> and it's split by single spacesspaces
								print 'Co-ordinates'
								...
								if destBlock == AIR: # OK to create a sign marker and update the command block
									text = []
									...
									print 'Unable to create a marker at %s %s %s. Is it air?' % ((int)(tx), (int)(ty), (int)(tz))
					...
					Text4 = t["Text4"].value
					print 'Found a sign %s %s %s %s' % (Text4,x,y,z)
	...
	for (SignAlias, x, y, z) in signAliasQueue:
		print 'Checking Sign %s' % (SignAlias)
