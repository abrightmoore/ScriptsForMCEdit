						...
						if command.find("minecraft:air") == -1: # @Texelelf
							...
							if command.find(ALIASPREFIX) == -1: # @Texelelf
								...
								if destBlock == AIR: # OK to create a sign marker and update the command block
									...
									# We found a sign that has already been generated, so I need to append this current alias to it. If there's no room, alarm! (to the console. Manual intervention required.
