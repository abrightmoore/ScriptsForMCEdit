...
def createSign(level, x, y, z, text): #abrightmoore - convenience method. Due to Jigarbov - this is not a Sign.
	ALIASKEY = "AJBLBLALIAS"
	COMMANDBLOCK = 137
	CHUNKSIZE = 16
	STANDING_SIGN = 63
	
	# setBlock(level, (STANDING_SIGN,8), x, y, z)
	setBlock(level, (COMMANDBLOCK,0), x, y, z)
	control = TAG_Compound()
	control["id"] = TAG_String("Control")
	control["CustomName"] = TAG_String(ALIASKEY)
	result = ''
	for i in text:
		result = result+' '+i
	control["Command"] = TAG_String(result)
...
def appendToSign(level, x, y, z, text): # I want to collapse the sign access later
	...
	for t in chunka.TileEntities:
		...
		y1 = t["y"].value
		z1 = t["z"].value
		if x == x1 and y == y1 and z == z1 and t["id"].value == "Control" and t["CustomName"].value == ALIASKEY: # Update this command block
			print 'Updating text %s %s %s with %s' % (x1,y1,z1,text)
			theCurrentText = t["Command"].value.split()
			t["Command"].value = t["Command"].value + ' ' + text
			chunka.dirty = True
			return True # Success
	return False # Fail
		...
		for t in chunk.TileEntities:
			...
			if x >= box.minx and x < box.maxx and y >= box.miny and y < box.maxy and z >= box.minz and z < box.maxz:
				if t["id"].value == "Control":
					...
					if (command.find(CBCOMMAND) != -1 or command.find(CBCOMMAND2) != -1) and t["CustomName"].value != ALIASKEY: # after @Texelelf
						...
						if True: # command.find("minecraft:air") == -1: # @Texelelf
							if command.find(ALIASPREFIX) == -1: # @Texelelf
								...
								if destBlock == AIR or (destBlock != AIR and OVERWRITE == True and destBlock != COMMANDBLOCK): # OK to create a sign marker and update the command block
									text = []
									...
									if s == False:
										print 'WARNING: Unable to append and Alias to the (possible) marker at %s %s %s. We may be out of sync. Please review.' % ((int)(tx), (int)(ty), (int)(tz))
									else:
										t["Command"].value = newCmd
										chunk.dirty = True
								else:
					...
					elif t["CustomName"].value == ALIASKEY: # This is a special block whose purpose is to store alias markers
						text = command.split()
						for Text4 in text:
							print 'Found a marker with alias %s %s %s %s' % (Text4,x,y,z)
							# This is an alias sign. Ignore if this is newly created.
							found = False
							for (alias, x, y, z) in signNewQueue: # To Do - Do this as a dictionary for speed
								if Text4 == alias:
									found = True
							if found == False:
								blob = (Text4,x,y,z)
