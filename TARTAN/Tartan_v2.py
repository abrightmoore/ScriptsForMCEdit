...
def Tartan(level, box, options):
	...
	for loop in xrange(0,3):
		for i in xrange(0,len(o)): # initialise
			#print i
			if o[i] == True:
				c.append(i)
	if len(c) == 0:
...
#		counter += 1
#		crandom.append(c.pop(randint(0,len(c)-1)))
#	c = crandom
	...
	for x in xrange(0,width):
		...
		maxZBlock = c[0]
		theBlock = c[0]
		for ix in xrange(0,len(c)*2-1):
			valueX = sin(x/centreWidth*period[ix%(len(c)-1)]*twoPi)
			if valueX > maxXValue:
				maxXValue = valueX
				maxXBlock = ix
		theBlock = maxXBlock
	...
	# Final pass - re-map the 3D structure as a 2D projections to the top of the selection box
	for x in xrange(0,width):
		for z in xrange(0,depth):
			y = height-1
			while y >= 0:
				...
				if theBlock != AIR:
					setBlock(level, theBlock, x, height-1, z)
					y = -1
				y = y-1
