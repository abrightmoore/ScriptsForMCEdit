...
# v7 is for @rsmalec's orbital function
...
inputs = (
	...
	("Scale", 0.1),
	("Method", ("Render PNG","Draw Line","Block Model","Entity Block Model","Reddust Rainbow","Orbital")),
	("Line Start X", 0.5),
	("Line Start Y", 74.5),
	("Line Start Z", 0.5),
	("Line End X", 15.2),
...
def makePNGTicles(level, box, options): 
	...
	if METHOD == "Entity Block Model": # Create command blocks that creat particles alone a 3D line. Per @CitrioNMC
		MATERIALID = options["Material"].ID
		print '%s: Examining the selection box for material %s' % (method, MATERIALID)
			...
			for iterZ in xrange(0, depth):
				for iterX in xrange(0, width):
					...
					if tempBlock == MATERIALID: # We are in business! Man the hatches! Batten the mainsail! Create a particle command block
						...
						makePNGTicleCommandBlock(chunk,(cX,cY,cZ),(spawnerX,spawnerY,spawnerZ),PREFIX,SUFFIX,COORDS,True)
						packedSpawnerCount = packedSpawnerCount+1
						chunk.dirty = True
	...
	if METHOD == "Orbital": # Create command blocks that create commandblocks to do things in a circular plot
		print '%s: Calculating orbits!!!OOOOOO' % (method)
		
		radius = centreWidth
		centreX = box.minx+centreWidth
		centreY = box.miny+centreHeight
		centreZ = box.minz+centreDepth
		
		theta = 0.0
		phi = 0.0
		angle = 1.0/(radius*4) # one step in the orbit - a quarter block movement. 4 is magic
		print 'Angle step = %s' % (angle)
		MAXNUMBLOCKS = 2*pi/angle
		for iterSteps in xrange(0,(int)(MAXNUMBLOCKS)):
			print 'Step %s of %s' % (iterSteps,MAXNUMBLOCKS)
			# Calculate new X, Y, Z
			(cX,cY,cZ) = getRelativePolar((centreX,centreY,centreZ), (theta, phi, radius)) # Absolute coordinates
			(chunk, (spawnerX,spawnerY,spawnerZ)) = makePNGTicleCommandBlockSetup(level,box,options,packedSpawnerCount)
			makePNGTicleCommandBlock(chunk,(cX,cY,cZ),(spawnerX,spawnerY,spawnerZ),PREFIX,"",COORDS,False)
