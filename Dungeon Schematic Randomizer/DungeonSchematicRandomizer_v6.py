...
inputs = (
	  ...
	  ("Type", ("Underground","City","Tower","Pack It In")),
	  ("Schematic Set:", ("string","value=DungeonSchematics")),
	  ("Shape", ("Box","Cylinder","Sphere")),
	  ("Cladding", alphaMaterials.BlockofIron),
	  ("Stay in box?", True),
  	  ("Start in centre?", True),
...
def dungeonSchematicRandomizer(level, box, options):
	...
	STAYINBOX = options["Stay in box?"]
	STARTINCENTRE = options["Start in centre?"]
	...
	StartSchematicFiles = glob.glob("filters/"+DIRPATH+"/Start/*.schematic")
	...
	EncounterSchematicFiles = glob.glob("filters/"+DIRPATH+"/*.schematic")
	...
	for iterBuilding in xrange(0, BUILDINGS):
		...
		if STARTINCENTRE == True:
			x = box.minx+centreWidth
			z = box.minz+centreDepth
		...
		elif TYPE == "Pack It In":
			direction = 5 # To the stars!
		...
		while counter > 0:
			...
			if direction == 1: # North
				...
				for B in DUNGEONQ:
					if checkBoundingBoxIntersect(A, B) == True:
						result = True
				if result == False and ((STAYINBOX == True and x>box.minx and x<box.maxx and z>box.minz and z<box.maxz and y>box.miny and y<box.maxy) or STAYINBOX == False):
					if copyBlocksFromDBG(level,schematicNext, bbNext, cursorPosn) == True: # blit schematic in
						DUNGEONQ.append(A)
						placedATile = True
				else:
