...
def doDTM(level,box,options):
	print "Reading file..."
	(P,min,max) = DTMImport(options)
	print "Placing blocks..."
	renderHeightMap(level,box,options,P,min,max)

def renderHeightMap(level,box,options,P,MIN,MAX):
	Pw,Ph = P.shape
	...
	QUICKMODE = options["Quick Mode"]
	QUICKSAMPLER = options["Quick Sample Every"]
	readCursorRow = 0
	if box.miny + MAX > box.maxy:
		print "Render exceeds selection box limit. Truncating result: "+str(MAX)+" > "+str(box.maxy-box.miny)
		MAX = box.maxy-box.miny
		...
		if writeCursorRow - box.minz < Ph: # Don't go past the row limit in source data
			...
			for writeCursorCol in xrange(box.minx, box.maxx):
				if writeCursorCol - box.minx < Pw: # Don't go past the column limit in source data
					if QUICKMODE == False or (QUICKMODE == True and readCursorCol % QUICKSAMPLER == 0 and readCursorRow % QUICKSAMPLER == 0):
						heightHere = box.miny+int(SCALEVERT*P[readCursorRow][readCursorCol])+DEPTHBASE
						if heightHere >= box.maxy:
							heightHere = box.maxy-1
						#if writeCursorCol%100 == 0:
						#	print heightHere
						for y in xrange(box.miny,int(heightHere)):
							if y < heightHere-DEPTHSOIL and QUICKMODE == False:
								setBlockForced(level,MAT_GROUND,writeCursorCol,y,writeCursorRow)
							elif y < heightHere-1 and QUICKMODE == False:
