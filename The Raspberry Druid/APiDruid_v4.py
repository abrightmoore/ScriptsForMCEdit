...
def APiDruid(level, box, options): 
	...
	for cycling in xrange(0, ITERATIONS):
		...
		# Paths...
		dirtPos = mc.player.getPos()
		blockBeneath = mc.getBlockWithData((int)(dirtPos.x+OFFSET),(int)(dirtPos.y-1),(int)(dirtPos.z+OFFSET))
		if blockBeneath == block.DIRT or blockBeneath == block.GRASS:
			mc.setBlock((int)(dirtPos.x+OFFSET),(int)(dirtPos.y-1),(int)(dirtPos.z+OFFSET),block.FARMLAND)
		# end Paths
			...
			if dice < 10:
				...
				if dice < 10:
					...
					if (tgtBlock == block.AIR or tgtBlock == block.SNOW) and (tgtBlockBeneath != block.AIR and tgtBlockBeneath != block.SNOW):
						...
						interTaskVar1 = randint(5,15)
						mc.postToChat("From little things...")

				interTaskCounter = 0
		
		if currentTask == "Grow a Tree":
			CANOPYHEIGHT = interTaskVar1
