...
#			"EnderDragon"
	...
	for rider in MOBS:
		...
		if rider == "Ghast":
			offset = "-5"
		theCommand = "summon "+rider+" ~"+offset+" ~2 ~ {NoAI:1,CustomName:Rider"+rider+",Invulnerable:1,PersistenceRequired:1,direction:[90.0,0.0,0.0],Motion:[0.0,0.0,0.0],Riding:{id:ArmorStand,Marker:1,NoGravity:1,Invulnerable:1},Template:1}"
	...
	for rider in MOBS:
		...
		for ridden in MOBS:
			...
			setBlock(level, COMMANDBLOCK, dX, dY, dZ)
			if (iterX+iterZ)%2 == 0:	
				setBlock(level, (fillMaterialBlock,iterX%16), dX, dY+1, dZ)
			else:
				setBlock(level, (edgeMaterialBlock,iterX%16), dX, dY+1, dZ)
			setBlock(level, STONE_BUTTON, dX, dY+2, dZ)
			iterX = iterX + 1
		iterZ = iterZ+1
