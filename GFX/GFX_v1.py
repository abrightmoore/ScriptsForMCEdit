...
def DeathStar2(level, box, options):
	...
	for iterY in xrange(0,height):
		...
		for iterX in xrange(0, width):
			...
			for iterZ in xrange(0, depth):
				...
				if posn >= 0 and posn < 64: # this block is on the sphere surface
					if dz > 0 and (iterY%3 == 0 or iterZ%5 == 0):
						randomDepth = randint(0,2*dz) # percent depth to render
	...
	for i in xrange(0,t):
		...
		horizAngle2 = angleSize * randint(0,360)
		...
		while horizAngle <= horizAngle2:
			p2 = getRelativePolar(p1, ( horizAngle, vertAngle, r+1))
			horizAngle = horizAngle + angleSize
			p3 = getRelativePolar(p1, ( horizAngle, vertAngle, r+1))
			drawLine(level, (fillMaterialBlock, fillMaterialData), p3, p2)

		
	vertAngle = 0
	vertAngles = [angleSize,-angleSize,2*angleSize,-2*angleSize] #,3*angleSize,-3*angleSize]
	...
	for i in xrange(0,t):
		...
		vertAngle2 = angleSize * randint(0,360)
	...
	size = r/2
	radius = r+(size*3/4)
	(x, y, z) = getRelativePolar(p1, (-pi/2,pi/6, radius))
