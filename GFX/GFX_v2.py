...
def DeathStar2(level, box, options):
	...
	DeathStarRemoveBack(level, box, options, r)
	...
	for s in xrange(5, randint(1,r)):
		...
		for t in xrange(10, r):
			u = randint(5,10)
			p2 = getRelativePolar(p1, (theta, phi, r))
			p3 = getRelativePolar(p1, (theta, phi+u, r))
			drawLine(level, AIR, p3, p2)
			p2 = getRelativePolar(p1, (theta, phi, r-1))
			p3 = getRelativePolar(p1, (theta, phi+u, r-1))
			drawLine(level, AIR, p3, p2)
			p2 = getRelativePolar(p1, (theta, phi, r-2))
			p3 = getRelativePolar(p1, (theta, phi+u, r-2))
			drawLine(level, AIR, p3, p2)
...
def DeathStarRemoveBack(level, box, options, r):
	# draw a spherical object reminiscent of a certain non-moon
	# for each 'layer' draw a random line from the edge of the sphere at this point to random depth
	method = options["Operation"]
	print '%s: Started at %s' % (method, time.ctime())
...
def DeathStarBands(level, box, options, r):
	...
	for vertAngle in vertAngles:
		print '%s: Central horizontal band %s of %s' % (method, i, t)
		horizAngle = 0
