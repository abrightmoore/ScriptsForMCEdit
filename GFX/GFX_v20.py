...
def InteriorDesign(level, box, options):
	...
	floorHeight = -1 # default is 'no floor found'
	for iterY in xrange(0, height):
		tempBlock = (schematic.blockAt(centreWidth,iterY,centreDepth), schematic.blockDataAt(centreWidth,iterY,centreDepth))
		if tempBlock != AIR: # Assume it is solid.
			tempBlock = (schematic.blockAt(centreWidth,iterY+1,centreDepth), schematic.blockDataAt(centreWidth,iterY+1,centreDepth))
			if tempBlock == AIR:
				tempBlock = (schematic.blockAt(centreWidth,iterY+2,centreDepth), schematic.blockDataAt(centreWidth,iterY+2,centreDepth))
				if tempBlock == AIR: # 2m of space above this block. Might be a room...
					floorHeight = iterY
					break
	if floorHeight == -1:
		print 'No floor found, unable to design interior :['
		return False
	...
	if carpetStyle > 90:
		(x,y,z) = (centreWidth, floorHeight+1, centreDepth)
		MAXLENGTH = (width+depth) / 2
		length = randint(5, MAXLENGTH)
		dir = 1
		InteriorDesign_spiralCarpet(schematic,pallette,x,y,z,dir,length)
		InteriorDesign_spiralCarpet(schematic,pallette,x,y,z-1,dir+2,length)
	else:
		(x,y,z) = (centreWidth, floorHeight+1, centreDepth)
		MAXLENGTH = (width+depth) / 2
		length = randint(5, MAXLENGTH)
		dir = 1
		InteriorDesign_chequerCarpet(schematic,pallette,x,y,z,dir,length)
		InteriorDesign_chequerCarpet(schematic,pallette,x,y,z-1,dir+2,length)
...
def InteriorDesign_spiralCarpet(schematic,pallette,x,y,z,dir,length):
	...
	for iterR in xrange(0, length):
		for steps in xrange(0,iterR-1):
			...
			if tempBlock == AIR:
				setBlock(schematic,(MAT_CARPET_ID,(int)(pallette[CARPETCOL])),x,y,z)
			else:
				iterR = length
				break
			if dir == 1:
...
def InteriorDesign_chequerCarpet(schematic,pallette,x,y,z,dir,length):
	# Magic numbers for block IDs
	MAT_CARPET_ID = 171 # Is this in a new Materials file?
	AIR = (0,0)
	...
	for iterR in xrange(0, length):
		for steps in xrange(0,iterR-1):
			...
			if tempBlock == AIR:
				if (x+z)%3 == 0:
					setBlock(schematic,(MAT_CARPET_ID,(int)(pallette[CARPETCOL1])),x,y,z)
				if (x+z)%3 == 1:
					setBlock(schematic,(MAT_CARPET_ID,(int)(pallette[CARPETCOL2])),x,y,z)
				if (x+z)%3 == 2:
					setBlock(schematic,(MAT_CARPET_ID,(int)(pallette[CARPETCOL3])),x,y,z)				
			else:
				iterR = length
				break
			if dir == 1:
